...
const { id } = useParams();
  ...
  return (
    <section className='single-product'>
      <h1 className='text-center text-white text-4xl uppercase'>discover our product</h1>
        ...
        <div className='grid grid-cols-2 text-white'>
          <div>
            ...
            <div className='flex flex-col text-center justify-start items-center gap-10 py-20'>
              <h2 className='text-6xl font-semibold s-product-name'>{product.name}</h2>
              <p className='text-3xl'>Price: ${product.price}.00</p>
              ...
              <button 
                  onClick={() =>  add(product)}
                  className='s-product-btn capitalize'>add to cart</button>
            ...
            <div className='flex flex-col gap-10 specs'>
              <h1 className='capitalize text-4xl text-white text-center'><span className='font-semibold s-product-name'>{product.name}</span> - premium wirless noise cancelation headphones</h1>
                ...
                {specs.map((spec) => {
                  ...
                  return (
                    <div key={id} className='flex flex-col text-center text-white specs-info py-6 px-3 justify-center rounded-md'>
                      <span className='text-6xl mb-4 spec-icon'>{icon}</span>
                      <h4 className='capitalize text-2xl'>{title}</h4>
                    </div>
                  )
                })}
              </div>
